...
TetrisState Tetris::accept(char key) {
  ...
  if (state != op->preState) { // 잘못된 키
    cout << "wrong preState for the current key!" << endl;
    return state;
  }
  op->hAction->run(this, key); // 동작수행
  Matrix *tempBlk = overlap_currBlk();
  if (anyConflict(tempBlk) == false) { // 충돌 없으면 postAstate로
    state = op->postAState;
  }
  else {
    //cout << "conflict" << endl;
    op->hCounterAction->run(this, key); // 충돌있으면 반대동작 후 수행 postCstate로
    delete tempBlk;
