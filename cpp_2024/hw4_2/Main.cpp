...
int main(int argc, char *argv[]) {
  ...
  TetrisState state;
  TetrisState Lstate, Rstate;
  CTetris::init(setOfColorBlockArrays, MAX_BLK_TYPES, MAX_BLK_DEGREES);

  CTetris *Lboard = new CTetris(10, 10);
  CTetris *Rboard = new CTetris(10, 10);

  key = getTetrisKey(TetrisState::NewBlock, fromUser, toFile);
  Lstate = Lboard->accept(key, NULL, NULL);
  Rstate = Rboard->accept(key, NULL, NULL);

  drawScreen(Lboard->get_oCScreen(), Lboard->get_wallDepth(), &left_win); 
  drawScreen(Rboard->get_oCScreen(), Rboard->get_wallDepth(), &rght_win); 
  
  Matrix* deletedLines = nullptr;

  while (true) {
    key = getTetrisKey(Lstate, fromUser, toFile);
    if (key == 'q') break;

    if (key == 'a' || key == 'w' || key == 'd' || key == 's' || key == ' ') {
        if (Lstate != TetrisState::Finished) {
            Lstate = Lboard->accept(key, nullptr, &deletedLines);
            drawScreen(Lboard->get_oCScreen(), Lboard->get_wallDepth(), &left_win);

            if (deletedLines != NULL){
              cout << "something deleted" << endl;
              Rboard->accept('N', deletedLines, NULL);
              cout << "line 350" << endl;
              drawScreen(Rboard->get_oCScreen(), Rboard->get_wallDepth(), &rght_win);
              delete deletedLines;
              //deletedLines = NULL;
            ...
            if (Lstate == TetrisState::NewBlock) {
                key = getTetrisKey(Lstate, fromUser, toFile);
                if (key == 'q') break;
                Lstate = Lboard->accept(key, nullptr, &deletedLines);
                drawScreen(Lboard->get_oCScreen(), Lboard->get_wallDepth(), &left_win);
                if (Lstate == TetrisState::Finished) break;
            }
        }
    } else if (key == 'j' || key == 'i' || key == 'l' || key == 'k' || key == '\r') {
        if (Rstate != TetrisState::Finished) {
            Rstate = Rboard->accept(key, nullptr, &deletedLines);
            drawScreen(Rboard->get_oCScreen(), Rboard->get_wallDepth(), &rght_win);

            if (deletedLines != NULL) {
                Lboard->accept('N', deletedLines, NULL);
                drawScreen(Lboard->get_oCScreen(), Lboard->get_wallDepth(), &left_win);
                delete deletedLines;
                //deletedLines = NULL;
            }

            if (Rstate == TetrisState::NewBlock) {
                key = getTetrisKey(Rstate, fromUser, toFile);
                if (key == 'q') break;
                Rstate = Rboard->accept(key, nullptr, &deletedLines);
                drawScreen(Rboard->get_oCScreen(), Rboard->get_wallDepth(), &rght_win);
                if (Rstate == TetrisState::Finished) break;
            }
        }
    }

    if(deletedLines != NULL) delete deletedLines;
    // Redraw both screens in case both boards need updates
    drawScreen(Lboard->get_oCScreen(), Lboard->get_wallDepth(), &left_win);
    drawScreen(Rboard->get_oCScreen(), Rboard->get_wallDepth(), &rght_win);
}
