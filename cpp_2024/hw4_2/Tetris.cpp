...
Matrix *deleteFullLines(Matrix *screen, Matrix *blk, int top, int dw, Matrix *prevScreen, Matrix **out) {
    ...
    cout << "133 line, in deleteFullLines" << endl;
    ...
    if (nDeleted > 0) {
        cout << "nDeleted > 0" << endl;
        cout << Dstart << " " << dw << " " << nDeleted << " " << dw + ws_dx << endl;

        Matrix *clipped = prevScreen->clip(Dstart - nDeleted + 1, dw, Dstart + 1, dw + ws_dx);
        cout << *clipped << endl; // clipped 객체 내용 출력

      //   if (*out != nullptr) {
      //       delete *out; // 기존 할당된 메모리 해제
      //   }
      // *out = new Matrix(clipped); // out이 가리키는 포인터가 clipped를 가리키도록 설정

      //cout << "out: " << *out << endl; // out이 가리키는 clipped 객체 출력
      delete clipped;
    }
...
TetrisState Tetris::accept(char key, Matrix *in, Matrix **out) {
  ...
  if (state == TetrisState::Finished)
    ...
    switch (key) { // perform the requested action
      case 'a': case 'j': left--; break;
      case 'd': case 'l': left++; break;
      case 'w': case 'i':
        degree = (degree + 1) % numDegrees; 
        currBlk = setOfBlockObjects[type][degree]; 
        break;
      case 's': case 'k': top++; break;
      case ' ': case '\r': 
        while (true) {
          top++;
      ...
      case 'N':
            cout << "N pressed" << endl;
            if(in != nullptr){
              int length = in->get_dy();
              int ws_dy = oScreen->get_dy() - get_wallDepth();
              int ws_dx = oScreen->get_dx() - 2*get_wallDepth();

              Matrix *existed = oScreen->clip(top, 0, ws_dy, ws_dx);
              oScreen->paste(existed, top-length, 0);
              delete existed;
              cout << "pasted up" <<endl;
              oScreen->paste(in, ws_dy - length, 0);
              return state;
            }
            break;
    ...
    if (tempBlk2->anyGreaterThan(1)) {
      switch (key) { // undo the requested action
        ...
        case 'w' : case 'i': 
          ...
          currBlk = setOfBlockObjects[type][degree]; 
          break;
        case 's' : case 'k': top--; touchDown = true; break;
        case ' ': case '\r': top--; touchDown = true; break;
      }
      delete tempBlk2;
      tempBlk = iScreen->clip(top, left, top + currBlk->get_dy(), left + currBlk->get_dx());
      tempBlk2 = tempBlk->add(currBlk);    
    ...
    if (touchDown) {
      oScreen = deleteFullLines(oScreen, currBlk, top, wallDepth, iScreen, out);
