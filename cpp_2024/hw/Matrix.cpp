...
{
  if (this == &obj) return *this; // 복사할 것이 자기자신이면 자기자신 리턴
  if ((dx != obj.dx) || (dy != obj.dy)) {  // 크기가 다르면.. 대입 못해
    if (array != NULL) dealloc();  // 현재 있는 것을 삭제
    alloc(obj.dy, obj.dx); // 복사 대상과 같은 크기로 자기자신 할당
  }
  for (int y = 0; y < dy; y++)  // 자기자신에 복사 내용을 채워넣음
    for (int x = 0; x < dx; x++)
      array[y][x] = obj.array[y][x];
  return *this; // 리턴 자기자신
}
